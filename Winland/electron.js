...
fs.writeFileSync(PS1_BLUETOOTH, [
  '$connected = @{}',
  '$ignore = "Enumerator|Service|Protocol|Transport|Attribute|Adapter|Controller|Hub|Root|Interface|Composite|Gateway|Push|Access|Serial|LE|RFCOMM|Intel|Microsoft|Realtek|Bluetooth|Volume|High Definition|Stereo Mix|Virtual|Steam|NVIDIA|DisplayAudio|Odyssey|Microphone|Speakers|Headphones|Headset"',
  '$pnp = Get-PnpDevice -PresentOnly -Class Bluetooth, WPD, MEDIA, AudioEndpoint -ErrorAction SilentlyContinue | Where-Object { $_.Status -eq "OK" -and $_.FriendlyName }',
  'foreach ($d in $pnp) {',
  '  $rawName = $d.FriendlyName',
  '  $cleanName = ($rawName -replace "\\s*(Avrcp Transport|Hands-Free.*|AG.*|HF.*|Audio.*|A2DP.*|Pse Service.*)", "").Trim()',
  '  if ($cleanName -and $cleanName -notmatch $ignore) {',
  '    if (-not $connected.ContainsKey($cleanName)) {',
  '      $devType = if ($cleanName -match "Galaxy|S24|S25|S26|iPhone|Pixel|OnePlus|Xiaomi|Phone") { "phone" } else { "audio" }',
  '      $connected[$cleanName] = @{ id = $cleanName; name = $cleanName; bat = -1; type = $devType }',
  '    }',
  '  }',
  '}',
