...
function useAlbumPalette(artworkUrl) {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      const lerp = (a, b) => a + (b - a) * FADE_SPEED;
      cur.primary = [lerp(cur.primary[0], tgt.primary[0]), lerp(cur.primary[1], tgt.primary[1]), lerp(cur.primary[2], tgt.primary[2])];
      cur.secondary = [lerp(cur.secondary[0], tgt.secondary[0]), lerp(cur.secondary[1], tgt.secondary[1]), lerp(cur.secondary[2], tgt.secondary[2])];
      paletteRef.current = { primary: rgbToCss(...cur.primary), secondary: rgbToCss(...cur.secondary) };
...
// ---- Now-Playing audio visualizer -----------------------------------------
// Uses real FFT band data streamed from the WASAPI loopback native addon
// (Windows only — see native/wasapi-loopback) via window.electronAPI.audio.
// On platforms/builds where that addon isn't available, it falls back to a
// smooth play-state-driven animation instead of pretending to analyze audio
...
export default function Island() {
  ...
  const albumPaletteRef = useAlbumPalette(spotifyTrack?.artwork_url || null);
    ...
    : (mode === "quick" && isPlaying && !alert && !chargingAlert && !bluetoothAlert && !cameraAlert && !microphoneAlert)
      ...
      : (mode === "quick" || alert || chargingAlert || bluetoothAlert || cameraAlert || microphoneAlert)
        ...
        : isPlaying
          ? nowPlayingWidth
          : 170;
  let height = mode === "large" ? (currentTab === 7 ? (positionMode === "free" ? 425 : 345) : currentTab === 6 ? 250 : currentTab === 3 ? 215 : currentTab === 0 ? 120 : currentTab === 1 ? 210 : 190) : 40;
