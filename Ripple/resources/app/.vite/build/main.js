...
@($results) | ConvertTo-Json -Compress -Depth 2
`,"utf16le").toString("base64");l(`powershell -NoProfile -EncodedCommand ${e}`,{maxBuffer:2*1024*1024},(s,r)=>{if(s||!r)return n([]);try{const o=JSON.parse(r.trim());n(Array.isArray(o)?o:o?[o]:[])}catch{n([])}})})}async function V(){const[n,t]=await Promise.all([z(),H()]),e=new Set,s=[];for(const r of[...n,...t]){if(!r.name||!(r.path||r.appId))continue;const o=r.type==="uwp"?`shell:AppsFolder\\${r.appId}`:r.path,a=o.toLowerCase();e.has(a)||(e.add(a),s.push({name:r.name,launch:o}))}return s.sort((r,o)=>r.name.localeCompare(o.name))}function M(n){const t=[];let e=0;for(;e<n.length;){for(;e<n.length&&/\s/.test(n[e]);)e++;if(e>=n.length)break;let s="";for(;e<n.length&&!/\s/.test(n[e]);)if(n[e]==='"'){for(e++;e<n.length&&n[e]!=='"';)s+=n[e++];e<n.length&&e++}else s+=n[e++];s&&t.push(s)}return t}function Q(n){const t=n.replace(/\//g,"\\").replace(/%([^%]+)%/g,(o,a)=>process.env[a]||`%${a}%`),e=t.match(/^"([^"]+)"(.*)/);if(e)return{exe:e[1],args:e[2].trim()?M(e[2].trim()):[]};const s=t.match(/^(.+?\.(?:exe|cmd|bat|com|ps1))(?:\s+(.*))?$/i);if(s)return{exe:s[1],args:s[2]?M(s[2]):[]};const r=t.search(/\s/);return r===-1?{exe:t,args:[]}:{exe:t.slice(0,r),args:M(t.slice(r+1).trim())}}function X(n){let t=n.trim();if(t.toLowerCase().includes("spotify")){T.openExternal("spotify:").catch(()=>{l('start "" spotify:')});return}if(!t.startsWith("shell:")&&t.includes("!")&&(t=`shell:AppsFolder\\${t}`),t.startsWith("shell:")){const e=t.replace(/'/g,"''");l(`powershell -NoProfile -WindowStyle Hidden -Command "Start-Process '${e}'"`);return}if(/[\\\/]/.test(t)){const{exe:e,args:s}=Q(t);if(s.length===0){if(e.toLowerCase().endsWith(".url")){try{const m=d.readFileSync(e,"utf8").match(/^URL=(.+)$/im);m&&T.openExternal(m[1].trim())}catch{}return}T.openPath(e).then(a=>{a&&l(`start "" "${e}"`)});return}const r=/[\\/]/.test(e)&&!/\.[^\\.]+$/.test(e)?e+".exe":e;if(/\.(cmd|bat)$/i.test(r)){const a=j("cmd.exe",["/c",r,...s],{shell:!1,detached:!0,stdio:"ignore"});a.on("error",()=>{}),a.unref();return}if(/\.ps1$/i.test(r)){const a=j("powershell.exe",["-NoProfile","-ExecutionPolicy","Bypass","-File",r,...s],{shell:!1,detached:!0,stdio:"ignore"});a.on("error",()=>{}),a.unref();return}const o=j(r,s,{shell:!1,detached:!0,stdio:"ignore"});o.on("error",()=>{}),o.unref();return}if(t.includes(" ")){const e=t.replace(/'/g,"''");l(`powershell -NoProfile -WindowStyle Hidden -Command "Start-Process '${e}'"`)}else l(`start "" ${t}`)}u.handle("set-ignore-mouse-events",(n,t,e)=>{i&&(process.platform!=="linux"?i.setIgnoreMouseEvents(t,{forward:e||!1}):i.setIgnoreMouseEvents(t))});u.handle("focus-window",()=>{i&&i.focus()});u.handle("open-external",async(n,t)=>{await T.openExternal(t)});u.handle("launch-app",async(n,t)=>{var r;const e=process.platform,s=(t||"").trim().toLowerCase();if(s.includes("spotify"))try{await T.openExternal("spotify:");return}catch{}if(e==="darwin")l(`open -a "${t}"`);else if(e==="win32"){let o=null;try{const a=p.join(c.getPath("userData"),"app-cache.json");if(d.existsSync(a)){const f=JSON.parse(d.readFileSync(a,"utf8")).filter(S=>S.name&&S.name.toLowerCase()===s);o=((r=f.find(S=>!S.launch.startsWith("shell:"))||f[0])==null?void 0:r.launch)||null}}catch{}X(o||t)}else l(t)});u.handle("build-app-cache",async()=>{if(process.platform!=="win32")return;const n=p.join(c.getPath("userData"),"app-cache.json");try{const t=await V();d.writeFileSync(n,JSON.stringify(t))}catch{}});u.handle("search-apps",async(n,t)=>{if(process.platform!=="win32"||!t)return[];const e=p.join(c.getPath("userData"),"app-cache.json");try{if(!d.existsSync(e))return[];const s=JSON.parse(d.readFileSync(e,"utf8")),r=t.toLowerCase();return s.filter(o=>o.name&&o.name.toLowerCase().includes(r)).slice(0,8)}catch{return[]}});u.handle("get-displays",()=>N.getAllDisplays().map(t=>({id:t.id,label:t.label||`Display ${t.id}`,bounds:t.bounds})));u.handle("set-display",(n,t)=>{if(i){const s=N.getAllDisplays().find(f=>f.id.toString()===t.toString())||N.getPrimaryDisplay(),{x:r,y:o,width:a,height:m}=s.bounds;process.platform,i.setBounds({x:r,y:o,width:a,height:m}),i.show()}});u.handle("update-window-position",(n,t,e)=>{});u.handle("set-auto-launch",(n,t)=>{if(process.platform==="linux"){const e=p.join(c.getPath("home"),".config","autostart"),s=p.join(e,"ripple.desktop");try{if(t){d.existsSync(e)||d.mkdirSync(e,{recursive:!0});const r=`[Desktop Entry]
Type=Application
Version=1.0
Name=Ripple
...
return 'null'
`;try{(!n.existsSync(e)||n.readFileSync(e,"utf8")!==s)&&n.writeFileSync(e,"\uFEFF"+s,"utf8")}catch{}return e}async function ee(n,t,e){if(n==="Unknown Artist"||t==="Unknown Title")return null;const s=`${n}-${t}-${e||""}`.toLowerCase();return P.has(s)?P.get(s):new Promise(r=>{const a=`https://itunes.apple.com/search?term=${encodeURIComponent(`${n} ${t} ${e||""}`.trim())}&entity=song&limit=10`,f=require("https").get(a,{timeout:3500},$=>{let S="";$.on("data",C=>S+=C),$.on("end",()=>{try{const k=JSON.parse(S).results||[];if(k.length>0){let x=null,A=-999;const F=t.toLowerCase(),G=n.toLowerCase(),v=(e||"").toLowerCase();for(const O of k){let b=0;const _=(O.trackName||"").toLowerCase(),B=(O.artistName||"").toLowerCase(),D=(O.collectionName||"").toLowerCase();(/dj mix|mixtape|today's hits|compilation|various artists/i.test(D)||/dj mix|mixed|remix/i.test(_))&&(b-=50),B.includes(G)&&(b+=30),_===F?b+=40:_.includes(F)&&(b+=20),v&&D.includes(v)&&(b+=50),b>A&&(A=b,x=O)}const L=(x||k[0]).artworkUrl100,R=L?L.replace("100x100bb","600x600bb"):null;return P.set(s,R),r(R)}}catch{}P.set(s,null),r(null)})});f.on("error",()=>{P.set(s,null),r(null)}),f.on("timeout",()=>{f.destroy(),P.set(s,null),r(null)})})}u.handle("control-system-media",async(n,t)=>{const e=process.platform;if(e==="darwin"){let s="";t==="playpause"?s=`tell application "System Events"
...
tell application "Music" to previous track
        ...
        $inUse
      "`,(s,r)=>{if(s)return n(!1);n(r.trim().toLowerCase()==="true")}):t==="linux"?l("fuser /dev/video* 2>/dev/null",(e,s)=>{n(s.trim().length>0)}):n(!1)}));u.handle("get-microphone-status",async()=>new Promise(n=>{const t=process.platform;t==="darwin"?l('ioreg -l | grep -E "IOAudioStreamActive|IOAudioEngine|IOAudioStream" | grep -i "Yes"',(e,s)=>{n(s?s.trim().length>0:!1)}):t==="win32"?l('powershell -NoProfile -Command "@(Get-ChildItem -Path "HKCU:\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\CapabilityAccessManager\\ConsentStore\\microphone" -Recurse -ErrorAction SilentlyContinue | ForEach-Object { Get-ItemProperty -Path $_.PSPath -Name "LastUsedTimeStop" -ErrorAction SilentlyContinue } | Where-Object { $_ -and $_.LastUsedTimeStop -eq 0 }).Count -gt 0"',(s,r)=>{if(s)return n(!1);n(r.trim().toLowerCase()==="true")}):t==="linux"?l("pactl list source-outputs | grep -q 'Source #'",e=>{n(!e)}):n(!1)}));c.on("window-all-closed",()=>{process.platform==="linux"&&!E&&c.quit()});const{Worker:te}=require("worker_threads");let w={available:!1,start:()=>!1,stop:()=>{}};try{if(process.platform==="win32"){const n=[p.join(process.resourcesPath||"","wasapi_loopback.node"),p.join(process.resourcesPath||"","wasapi-loopback.node"),p.join(c.getAppPath(),"resources","wasapi_loopback.node"),p.join(c.getAppPath(),"resources","wasapi-loopback.node"),p.join(__dirname,"../../resources/wasapi_loopback.node"),p.join(__dirname,"../../resources/wasapi-loopback.node"),p.join(__dirname,"../native/wasapi-loopback/build/Release/wasapi_loopback.node")];for(const t of n)try{if(d.existsSync(t)){const e=require(t);if(e&&typeof e.start=="function"){w=e,w.available=!0,console.log("[WASAPI] Loaded native loopback addon from:",t);break}}}catch(e){console.error("[WASAPI] Error loading candidate:",t,e)}}}catch(n){console.error("[WASAPI] Addon initialization error:",n)}let h=null,g=!1,I=0;function ne(){if(h)return h;try{const n=[p.join(process.resourcesPath||"","audioWorker.js"),p.join(c.getAppPath(),"resources","audioWorker.js"),p.join(__dirname,"../../resources/audioWorker.js")];let t=null;for(const e of n)if(d.existsSync(e)){t=e;break}return t?(h=new te(t),h.on("message",e=>{if((e==null?void 0:e.type)==="analysis"&&i&&!i.isDestroyed()){const s={bands:e.bands,beat:e.beat,bass:e.bass};i.webContents.send("audio-analysis",s),i.webContents.send("audio-bands",e.bands)}}),h.on("error",e=>{console.error("[AudioWorker] error:",e)}),h.on("exit",()=>{h=null,g=!1}),h):null}catch(n){return console.error("[AudioWorker] failed to start:",n),null}}u.handle("audio-viz-start",()=>{if(!w||!w.available&&typeof w.start!="function")return{started:!1,reason:"unsupported-platform"};if(I++,g)return{started:!0};const n=ne();return n?(g=!!w.start((e,s)=>{n.postMessage({type:"pcm",samples:e,sampleRate:s},[e.buffer])}),console.log("[WASAPI] Native audio capture started successfully:",g),{started:g}):{started:!1,reason:"worker-unavailable"}});u.handle("audio-viz-stop",()=>(I=Math.max(0,I-1),I===0&&g&&(w.stop(),g=!1,h&&h.postMessage({type:"reset"})),{stopped:!0}));c.on("before-quit",()=>{g&&w.stop(),h&&h.terminate()});let y={clientId:process.env.SPOTIFY_CLIENT_ID||"",clientSecret:process.env.SPOTIFY_CLIENT_SECRET||"",apiKey:process.env.SPOTIFY_API_KEY||""};function se(){try{const n=require("fs"),t=require("path"),e=t.join(c.getPath("userData"),"spotify-config.json"),s=t.join(c.getAppPath(),"spotify-config.json");let r=null;if(n.existsSync(e)?r=e:n.existsSync(s)&&(r=s),r){const o=JSON.parse(n.readFileSync(r,"utf-8"));y.clientId=o.SPOTIFY_CLIENT_ID||y.clientId,y.clientSecret=o.SPOTIFY_CLIENT_SECRET||y.clientSecret,y.apiKey=o.SPOTIFY_API_KEY||y.apiKey}}catch{}}u.handle("get-spotify-config",()=>(se(),y));u.handle("save-spotify-config",(n,t)=>{if(t){y={...y,...t};try{const e=require("fs"),r=require("path").join(c.getPath("userData"),"spotify-config.json");e.writeFileSync(r,JSON.stringify({SPOTIFY_CLIENT_ID:y.clientId,SPOTIFY_CLIENT_SECRET:y.clientSecret,SPOTIFY_API_KEY:y.apiKey,INSTRUCTIONS:"Spotify Web API Client ID & Client Secret"},null,2))}catch{}}return{success:!0}});
