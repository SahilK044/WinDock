...
Icon=${E()}
Terminal=false
`;d.writeFileSync(s,r)}else d.existsSync(s)&&d.unlinkSync(s)}catch(r){console.error("Failed to set auto-launch on Linux:",r)}}else if(process.platform==="win32")try{l.setLoginItemSettings({openAtLogin:n,path:l.getPath("exe")})}catch(e){console.error("Failed to set login item settings on Windows:",e)}});const E=()=>{const t="png";if(l.isPackaged){const n=y.join(process.resourcesPath,`icon.${t}`),e=y.join(process.resourcesPath,`assets/icons/icon.${t}`);return d.existsSync(n)?n:d.existsSync(e)?e:n}return y.join(__dirname,`../../src/assets/icons/icon.${t}`)},M=()=>{const t=A.getPrimaryDisplay(),{x:n,y:e,width:s,height:r}=t.bounds,o=process.platform==="linux",a=process.platform==="win32",p=process.platform==="darwin",f=s,g=r,m=n,b=e,C=a?"toolbar":"panel";i=new W({width:f,height:g,x:m,y:b,backgroundColor:"#00000000",transparent:!0,alwaysOnTop:!0,resizable:!1,frame:!1,...a?{}:{thickFrame:!1},hasShadow:!1,skipTaskbar:!0,icon:E(),...p?{hiddenInMissionControl:!0}:{},type:C,fullscreen:!1,visibleOnFullScreen:!0,acceptFirstMouse:!0,webPreferences:{preload:y.join(__dirname,"preload.js"),devTools:!1},show:!0}),o?i.setIgnoreMouseEvents(!0):i.setIgnoreMouseEvents(!0,{forward:!0});const _=o?500:0;i.once("ready-to-show",()=>{setTimeout(()=>{i&&(i.show(),o?i.setAlwaysOnTop(!0,"screen-saver"):i.setAlwaysOnTop(!0,"pop-up-menu"),i.focus())},_)}),setTimeout(()=>{i&&!i.isVisible()&&(i.show(),i.focus())},5e3),i.on("closed",()=>{i=null});try{i.setVisibleOnAllWorkspaces(!0,{visibleOnFullScreen:!0})}catch{}if(!l.isPackaged||process.env.NODE_ENV==="development")i.loadURL("http://localhost:5173");else{const N=y.join(__dirname,"../renderer/main_window/index.html");i.loadFile(N)}};l.whenReady().then(()=>{process.platform==="darwin"&&l.dock.hide(),M(),l.on("activate",()=>{W.getAllWindows().length===0&&M()});try{const t=E(),e=R.createFromPath(t).resize({width:16,height:16});k=new F(e);const s=j.buildFromTemplate([{label:"Show/Hide Ripple",click:()=>{i&&(i.isVisible()?i.hide():i.show())}},{type:"separator"},{label:"Quit",click:()=>{l.quit()}}]);k.setToolTip("Ripple"),k.setContextMenu(s)}catch(t){console.error("Failed to create tray:",t)}});const S=new Map;function G(){const t=require("fs"),e=require("path").join(l.getPath("userData"),"get_media.ps1"),s=`
...
return 'null'
`;try{(!t.existsSync(e)||t.readFileSync(e,"utf8")!==s)&&t.writeFileSync(e,s,"utf8")}catch{}return e}async function B(t,n){if(t==="Unknown Artist"||n==="Unknown Title")return null;const e=`${t}-${n}`.toLowerCase();return S.has(e)?S.get(e):new Promise(s=>{const o=`https://itunes.apple.com/search?term=${encodeURIComponent(`${t} ${n}`)}&entity=song&limit=1`,p=require("https").get(o,{timeout:2500},f=>{let g="";f.on("data",m=>g+=m),f.on("end",()=>{try{const m=JSON.parse(g);if(m.results&&m.results.length>0){const b=m.results[0].artworkUrl100,C=b?b.replace("100x100bb","600x600bb"):null;return S.set(e,C),s(C)}}catch{}S.set(e,null),s(null)})});p.on("error",()=>{S.set(e,null),s(null)}),p.on("timeout",()=>{p.destroy(),S.set(e,null),s(null)})})}u.handle("get-system-media",async()=>new Promise(t=>{const n=process.platform;if(n==="darwin")c(`osascript -e '
      ...
      "`,(s,r)=>{if(s)return t(!1);t(r.trim().toLowerCase()==="true")}):n==="linux"?c("fuser /dev/video* 2>/dev/null",(e,s)=>{t(s.trim().length>0)}):t(!1)}));u.handle("get-microphone-status",async()=>new Promise(t=>{const n=process.platform;n==="darwin"?c('ioreg -l | grep -E "IOAudioStreamActive|IOAudioEngine|IOAudioStream" | grep -i "Yes"',(e,s)=>{t(s?s.trim().length>0:!1)}):n==="win32"?c('powershell -NoProfile -Command "@(Get-ChildItem -Path "HKCU:\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\CapabilityAccessManager\\ConsentStore\\microphone" -Recurse -ErrorAction SilentlyContinue | ForEach-Object { Get-ItemProperty -Path $_.PSPath -Name "LastUsedTimeStop" -ErrorAction SilentlyContinue } | Where-Object { $_ -and $_.LastUsedTimeStop -eq 0 }).Count -gt 0"',(s,r)=>{if(s)return t(!1);t(r.trim().toLowerCase()==="true")}):n==="linux"?c("pactl list source-outputs | grep -q 'Source #'",e=>{t(!e)}):t(!1)}));l.on("window-all-closed",()=>{process.platform==="linux"&&!k&&l.quit()});u.handle("control-system-media",async(t,n)=>{const e=process.platform;if(e==="darwin"){const s=`
        ...
        `;c(`osascript -e '${s}'`)}else if(e==="linux"){let s=n;n==="playpause"&&(s="play-pause"),c(`playerctl ${s}`)}});const{Worker:Y}=require("worker_threads");let $=null;try{$=require("wasapi-loopback")}catch(t){console.error("[wasapi-loopback] module failed to resolve — has it been built? Reason:",t.message),$={available:!1,start:()=>!1,stop:()=>!1}}let w=null,P=!1,T=0;function J(){return l.isPackaged?y.join(process.resourcesPath,"audioWorker.js"):y.join(l.getAppPath(),"src","audio","audioWorker.js")}function K(){if(w)return w;const t=J();return d.existsSync(t)?(w=new Y(t),w.on("message",n=>{(n==null?void 0:n.type)==="bands"&&i&&!i.isDestroyed()&&i.webContents.send("audio-bands",n.bands)}),w.on("error",n=>{console.error("[audio-worker] error:",n)}),w):(console.error(`[audio-worker] script not found at ${t}`),null)}u.handle("audio-viz-start",()=>{if(!$.available)return{started:!1,reason:"unsupported-platform"};if(T++,P)return{started:!0};const t=K();if(!t)return{started:!1,reason:"worker-unavailable"};const n=$.start((e,s)=>{t.postMessage({type:"pcm",samples:e,sampleRate:s},[e.buffer])});return P=n,{started:n}});u.handle("audio-viz-stop",()=>(T=Math.max(0,T-1),T===0&&P&&($.stop(),P=!1,w&&w.postMessage({type:"reset"})),{stopped:!0}));l.on("before-quit",()=>{P&&$.stop(),w&&w.terminate()});let h={clientId:process.env.SPOTIFY_CLIENT_ID||"",clientSecret:process.env.SPOTIFY_CLIENT_SECRET||"",apiKey:process.env.SPOTIFY_API_KEY||""};function H(){try{const t=require("fs"),n=require("path"),e=n.join(l.getPath("userData"),"spotify-config.json"),s=n.join(l.getAppPath(),"spotify-config.json");let r=null;if(t.existsSync(e)?r=e:t.existsSync(s)&&(r=s),r){const o=JSON.parse(t.readFileSync(r,"utf-8"));h.clientId=o.SPOTIFY_CLIENT_ID||h.clientId,h.clientSecret=o.SPOTIFY_CLIENT_SECRET||h.clientSecret,h.apiKey=o.SPOTIFY_API_KEY||h.apiKey}}catch{}}u.handle("get-spotify-config",()=>(H(),h));u.handle("save-spotify-config",(t,n)=>{if(n){h={...h,...n};try{const e=require("fs"),r=require("path").join(l.getPath("userData"),"spotify-config.json");e.writeFileSync(r,JSON.stringify({SPOTIFY_CLIENT_ID:h.clientId,SPOTIFY_CLIENT_SECRET:h.clientSecret,SPOTIFY_API_KEY:h.apiKey,INSTRUCTIONS:"Spotify Web API Client ID & Client Secret"},null,2))}catch{}}return{success:!0}});
