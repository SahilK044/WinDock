...
Icon=${F()}
Terminal=false
`;d.writeFileSync(s,r)}else d.existsSync(s)&&d.unlinkSync(s)}catch(r){console.error("Failed to set auto-launch on Linux:",r)}}else if(process.platform==="win32")try{c.setLoginItemSettings({openAtLogin:n,path:c.getPath("exe")})}catch(e){console.error("Failed to set login item settings on Windows:",e)}});const F=()=>{const t="png";if(c.isPackaged){const n=p.join(process.resourcesPath,`icon.${t}`),e=p.join(process.resourcesPath,`assets/icons/icon.${t}`);return d.existsSync(n)?n:d.existsSync(e)?e:n}return p.join(__dirname,`../../src/assets/icons/icon.${t}`)},U=()=>{const t=I.getPrimaryDisplay(),{x:n,y:e,width:s,height:r}=t.bounds,o=process.platform==="linux",a=process.platform==="win32",f=process.platform==="darwin",m=s,S=r,w=n,C=e,k=a?"toolbar":"panel";i=new q({width:m,height:S,x:w,y:C,backgroundColor:"#00000000",transparent:!0,alwaysOnTop:!0,resizable:!1,frame:!1,...a?{}:{thickFrame:!1},hasShadow:!1,skipTaskbar:!0,icon:F(),...f?{hiddenInMissionControl:!0}:{},type:k,fullscreen:!1,visibleOnFullScreen:!0,acceptFirstMouse:!0,webPreferences:{preload:p.join(__dirname,"preload.js"),devTools:!1,backgroundThrottling:!1},show:!0}),o?i.setIgnoreMouseEvents(!0):i.setIgnoreMouseEvents(!0,{forward:!0});const A=o?500:0;i.once("ready-to-show",()=>{setTimeout(()=>{i&&(i.show(),o?i.setAlwaysOnTop(!0,"screen-saver"):i.setAlwaysOnTop(!0,"pop-up-menu"),i.focus())},A)}),setTimeout(()=>{i&&!i.isVisible()&&(i.show(),i.focus())},5e3),i.on("closed",()=>{i=null});try{i.setVisibleOnAllWorkspaces(!0,{visibleOnFullScreen:!0})}catch{}if(!c.isPackaged||process.env.NODE_ENV==="development")i.loadURL("http://localhost:5173");else{const T=p.join(__dirname,"../renderer/main_window/index.html");i.loadFile(T)}};c.whenReady().then(()=>{process.platform==="darwin"&&c.dock.hide(),U(),c.on("activate",()=>{q.getAllWindows().length===0&&U()});try{const t=F(),e=K.createFromPath(t).resize({width:16,height:16});O=new Y(e);const s=J.buildFromTemplate([{label:"Show/Hide Ripple",click:()=>{i&&(i.isVisible()?i.hide():i.show())}},{type:"separator"},{label:"Quit",click:()=>{c.quit()}}]);O.setToolTip("Ripple"),O.setContextMenu(s)}catch(t){console.error("Failed to create tray:",t)}});const b=new Map;function Z(){const t=require("fs"),e=require("path").join(c.getPath("userData"),"get_media.ps1"),s=`
[Console]::OutputEncoding = [System.Text.Encoding]::UTF8
...
return 'null'
`;try{(!t.existsSync(e)||t.readFileSync(e,"utf8")!==s)&&t.writeFileSync(e,"\uFEFF"+s,"utf8")}catch{}return e}async function ee(t,n,e){if(t==="Unknown Artist"||n==="Unknown Title")return null;const s=`${t}-${n}-${e||""}`.toLowerCase();return b.has(s)?b.get(s):new Promise(r=>{const a=`https://itunes.apple.com/search?term=${encodeURIComponent(`${t} ${n} ${e||""}`.trim())}&entity=song&limit=10`,m=require("https").get(a,{timeout:3500},S=>{let w="";S.on("data",C=>w+=C),S.on("end",()=>{try{const k=JSON.parse(w).results||[];if(k.length>0){let A=null,T=-999;const W=n.toLowerCase(),G=t.toLowerCase(),v=(e||"").toLowerCase();for(const x of k){let g=0;const N=(x.trackName||"").toLowerCase(),B=(x.artistName||"").toLowerCase(),D=(x.collectionName||"").toLowerCase();(/dj mix|mixtape|today's hits|compilation|various artists/i.test(D)||/dj mix|mixed|remix/i.test(N))&&(g-=50),B.includes(G)&&(g+=30),N===W?g+=40:N.includes(W)&&(g+=20),v&&D.includes(v)&&(g+=50),g>T&&(T=g,A=x)}const L=(A||k[0]).artworkUrl100,R=L?L.replace("100x100bb","600x600bb"):null;return b.set(s,R),r(R)}}catch{}b.set(s,null),r(null)})});m.on("error",()=>{b.set(s,null),r(null)}),m.on("timeout",()=>{m.destroy(),b.set(s,null),r(null)})})}u.handle("control-system-media",async(t,n)=>{const e=process.platform;if(e==="darwin"){let s="";n==="playpause"?s=`tell application "System Events"
